...
from apmodel.objects.actor import ActorEndpoints, Person
from apmodel.objects.document import Document
from apmodel.objects.hashtag import Hashtag
from apmodel.objects.note import Note
from apmodel.schema import PropertyValue


@pytest.fixture
def test_data_path(request: Any) -> Path:
...
def test_misskey_activity(test_data_path: Path):
    ...
    with open(data_loc) as f:
        ...
        assert activity.id == "https://misskey.example.com/notes/ag8g1pmyifww004j/activity"
        assert activity.actor == "https://misskey.example.com/users/afm74io04yxx0000"
        assert str(activity.published) == "2025-12-13 22:40:41.482000+00:00" or activity.dump()['published'] == "2025-12-13T22:40:41.482Z"
...
def test_fedibird_person(test_data_path: Path):
    ...
    with open(data_loc) as f:
        ...
        assert isinstance(actor.model_extra.get("searchableBy"), list) and actor.model_extra.get("searchableBy")[0] == "https://fedibird.example.com/users/user"
...
def test_akkoma_actor(test_data_path: Path):
    ...
    with open(data_loc) as f:
        ...
        assert actor.endpoints.model_extra.get("oauthAuthorizationEndpoint") == "https://akkoma.example.com/oauth/authorize"
...
def test_mastodon_note(test_data_path: Path):
    from apmodel.core import Collection
...
def test_akkoma_replies(test_data_path: Path):
    from apmodel.core import OrderedCollection, OrderedCollectionPage
